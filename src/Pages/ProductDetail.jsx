...
export default function ProductDetail() {
  ...
  return (
    <>   
    <div className="flex md:w-1/2 md:gap-4 md:p-5 mx-auto flex-col p-2">
      <img src={product.image} alt={product.name} className="md:w-1/2 w-full rounded shadow mb-4 " />
      <div className="flex flex-col gap-5 items-start ">
      <h1 className="text-2xl font-serif mb-2">{product.name}</h1>
      <p className="text-lg mb-4">{product.price}$</p>
      <p className="text-[12px]">Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quis corporis fugit hic ipsum 
      ...
      {related.length > 0 && (
        <div className="flex flex-wrap">
          ...
          <div className="flex  gap-4 items-center justify-center pb-3 flex-wrap">
            {related.map((rel) => (
              <Link to={`/shop/${rel.id}`} key={rel.id} className="block  rounded p-2 hover:shadow">
                ...
                <div className="text-center">
                  ...
                  <button className="btn btn-outline my-1.5">Add To Cart <i className="fa fa-shopping-cart"></i> </button>
                </div>
              </Link>
            ))}
          </div>
